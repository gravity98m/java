...
public class Ex2 {
	...
	public static void main(String[] args) {
		/*
		 * 메서드 오버로딩 시 문제점
		 * - 동일한 작업ㅇ르 수행하는 코드들의 중복이 발생
		 *   => 만약, 특정 메서드의 코드를 수정해야 할 경우
		 * 		동일한 코드를 여러번 수정해야하는 번거로움이 발생할 수 잇음
		 * 	 => 코드의 유지보수성이 나쁘다!
		 * 
		 * [해결책]
		 * - 오버로딩된 메서드에서 또 다른 오버로딩된 메서드를 호출하여
		 * 	 중복되는 작업을 대신 처리하도록 할 수 잇다.
...
class PrintClass{
	// 최대 3명까지의 초대인원에 대한 이름을 입력받아 출력하는 pring() 메서드 정의
	// => 단, 입력되지 않은 이름은 "없음"으로 출력
	// ex) print("홍길동","이순신","강감찬") 형태로 호출시
	//		초대멤버 :홍길동, 이순신, 강감찬 출력
...
//		System.out.println("초대멤버 :" + name1 + " 없음 없음");
		...
		// pring(String, String, String)메서드를 호출하여
