...
public class Test {
	...
	public static void main(String[] args) {
		...
		// => Object 타입으로 업캐스팅된 객체는 다시 다운캐스팅이 필요함
		...
		for(int i = 0; i < objArr.length; i++) {
			// Object[] 배열에 저장된 객체를 직접 다룰 경우 참조영역이 출소됨
			System.out.println(objArr[i]); // Person [name=홍길동, age=20] 형태로 출력
//			System.out.println(objArr[i].name); // Person 타입 변수 접근 불가
			
			// 배열 내의 객체를 꺼내서 Person 타입 변수 저장
//			Person p = objArr[i]; // 다운 캐스팅 필요!
			if(objArr[i] instanceof Person) {
				Person p = (Person)objArr[i];
				System.out.println(p.name + ", " + p.age); // 홍길동, 20 형태로 출력
			}
		}
		
//		Person[] pArr = {p1, p2}; // 위와 동일. 벗, 다운캐스팅하지 않고 강한 결합의 예
...
//		}
		 ...
		 *    => 배열을 단점인 크기 불변을 해결하게 되므로 객체 추가가 자유로움
		 * 1) 제네릭을 사용하지 않을 경우
		 *  - 파라미터 또는 리턴타입의 데이터타입이 Object 타입이 되어
		 *    다양한 객체를 저장 가능하게 됨
		 *    대신, 데이터를 꺼내는 시점에서 타입 불일치에 따른 오류 발생 가능
		 *  - 데이터를 꺼내기 전 instanceor 연산자를 통한 타입 판별 후 
		...
		// 문자열 "전지현" 추가
//		list.add("전지현"); // Object 타입이므로 Person이 아닌 타입도 추가 가능
		// Person 객체 형태로 꺼내서 사용하는 시점에서 문제가 발생할 수 있다.
		
		// 
//		for(int i = 0; i < list.size(); i++) {
////			Person p = list.get(i);
//			// => 제네릭 타잉을 사용하지 않아 List는 Object로 되어 있다.
//			// => Object 타입을 Person 타입으로 대입하려고 하면 컴파일 에러가 나며
//			//	  다움캐스팅이 필요함을 알 수 있다.
//			Person p = (Person)list.get(i);
//			System.out.println(p);
//			// => 실행을 시키면 82번 라인에서 에러가 발생한다.
//			// => class java.lang.String cannot be cast to class generic.Person
//			// => "전지현"의 String 형태는 Person 객체로 변경할 수 없음을 알 수 있다.
//			// => 위의 코드 중 list.add("전지현");을 주석처리하고 다시 실행하면
//			// 	  오류가 없어진다!
...
//			System.out.println(o);
			...
			// Person 타입으로 가져오기 전 타입 판별 수 형변환 수행
...
//		list2.add("전지현"); // 컴파일 에러 발생!
		...
		// 반복문을 통해 list2 객체의 모든 요소 꺼내기
		for(int i = 0; i < list2.size(); i++) {
			// get() 메서드 리턴 타입이 Person 타입이 되므로 형변환 불필요!
			// => 또한, 형변환 전 타입 판별도 생략
