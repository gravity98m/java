...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *      예외가 발생하면 JVM에 의해 해당 예외 객체를 전달받아
		 ...
		 *   (Compile Unchecked Exception 계열 : RntimeException 등
...
//		System.out.println(arr[5]);
		...
		// at exception.Ex.main(Ex.java:52)					 => 52번 라인에서 예외
		
//		String str = null;
//		System.out.println(str.length());
		// 객체의 주소를 참조하지 않는 참조변수(null) 접근 시 NullPointerException
		// java.lang.NullPointerException
		// at exception.Ex.main(Ex.java:58) => 58번 라인에서 예외 발생4
		
		// 프로그램 종료 문장은 예외가 발생하면 출력되지 못하고 강제 종료됨
		...
		try {
			...
			//     = RuntimeException 또는 Exception 클래스로 처리 가능
			
			System.out.println("try 블록 끝!");
		} catch(ArithmeticException e) {
			// ArithmeticException 예외가 발생했을 경우 처리할 코드들 기술
			// 예외가 발생하지 않을 경우 이 블록인 실행되지 않음
			e.printStackTrace(); // 예외 클래스, 발생 위치, 원인 등을 자세히 출력
//			System.out.println("피연산자가 0으로 되어 있어 에러 발생!");
