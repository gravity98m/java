...
//		Integer n2 = new Integer(20); // 정수 20을 갖는 인스턴스를 생성하고
		...
		// 기본데이터타입은 Stack공간에서 실제 데이터 형태를 직접 관리하지만
		// 객체는 Heap공간에 실제 데이터가 저장되고,
		// 해당 주소값을 Stack 공간에서 관리함.
		// 따라서, 기본데이터타입 변수와 참조데이터타입 변수는 호환이 불가능
		// 기본데이터타입 <-> 참조데이터타입 변환시 메서드 등을 활용해야했음.
...
//		System.out.println(num1 + ", " + n1 ); // 10, 10
		...
		// 메서드를 호출하는 변환 과정을 생략해도 자동ㅇ으로 변환을 수행하도록 변경됨
		// => 즉, 대입연산자를 통해 직접 대입이 가능하도록 바뀌었음
		n1 = num1; // 기본 데이터를 자동으로 Interger 타입으로 변환하여 저장
		// => 오토 박싱(Auto Boxing)
		System.out.println(num1 + ", " + n1 ); // 10, 10
		...
		int num3 = 10;
		Integer n3 = 20;
		System.out.println(num3 + n3); // 30
		// => 기본데이터타입과 Wrapper 클래스 타입간의 연산 수행할 경우
