...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * - 참조형 데이터타입간의 변환
		 * - 반드시 상속관계가 전제되어야 함
		 * 	 (= 상속 관계가 아닌 타입끼리 형변환은 허용되지 않음 = 오류발생!)
		 * - 슈퍼클래스 타입 <-> 서브클래스 타입
		 ...
		 * - 컴파일러에 의해 자동 형 변환이 일어남(=묵시적 형변환)
		 * 	 = 별도의 형변환 연산자를 사용하지 않아도 변환가능함(생략가능)
		 * - 참조가능한 영역이 축소됨
...
//		p.childPrn(); // 서브클래스에서 직접 정의한 메서드는 호출 불가!
		...
		// 슈퍼클래스 타입 Parent 의 레퍼런스 변수(p) 선언
		Parent p;
...
//		p.childPrn(); // 서브클래스에서 직접 정의한 메서드(접근 불가) => 오류 발생
		...
		//	  참조영역에 대한 축소로 인해 서브클래스 메서드가 보이지 않게 됨
		 ...
		 *    = 슈퍼클래스의 인스턴스를 서브클ㄹ래스 타입으로 형변환하는 것
		 * - 참조 가능한 영역이 확대됨 
		 ...
		 * ============================================================================
		 * - 다운캐스팅 후에도 실행 시 오류가 발생할 수 잇으므로 
		 *   인스턴스의 클래스 타입과 참조하는 레퍼런스 변수의
		 *   상속 관계를 고려하여 다운캐스팅을 수행해야 한다!
		 * 
		 * => 결론!
		 *    이전에 이미 업캐스팅된 인스턴스(레퍼런스)를 다시 다운캐스팅하는 경우에만 안전하다!
...
//		c2.childPrn();	// 실제 parent 인스턴스에 존재하지 않는 메서드
		// => 즉, 존재하지 않는 영역에 대한 참조 위험성 때문에 다운캐스티이 기본적으로 불가능함!
...
class Child extends Parent {
	// Child 클래스의 멤버 : 2개(상속받은 멤버와 자기자신의 멤버)
