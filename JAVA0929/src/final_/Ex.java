...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 * 1. 변수에 final이 붙을 경우
		 * 	  => 변수 값 변경 불가 = 지정되어 있는 값만 사용가능 = 상수로 취급
		 *    => 일반 변수와 구별하기 위해 이름을 보통 대문자로 사용하며, 
		 *       단어와 단어 사이의 구분은 언더스코어(_)를 사용함 ex) MAX_NUM
		 *    => 반드시 초기화 필수
		 *    	 단, 초기화를 수행하지 않는 상수를 blank final 상수라고 하며 생성자를 통해 초기화하면 사용이 가능함
...
class FinalMember{
	...
	public void change() {
//		finalMember = 90; // final 키워드가 붙은 변수(=상수)는 값 변경 불가!
...
class FinalMethod{
	...
	public void nomalMethod() { // final 메서드 = 오버라이딩 불가
...
class SubFinalMethod extends FinalMethod{
	...
	// final 메서드를 오버라이딩 살 수 없으므로 컴파일 에러 발생
//	@Override
...
//	}
	...
	public void nomalMethod() {
		System.out.println("서브클래스에서 오버라이딩된 nomalMethod()");
	}
}
...
//class SubStirng extends String {} // String 클래스 상속불가
//class SubStirng extends Math {} // Math 클래스 상속불가
