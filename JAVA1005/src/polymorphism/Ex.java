...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   실행 대상이 달라지는 것
		 * - 참조변수의 타입과 무관하게 실제 인스턴스의 메서드를 실행하는 것
		 */
		...
		p = c; // Child -> Parent
		// => Parent p = new Child();
		p.parentPrn(); // Child 인스턴스의 오버라이딩된 메서드가 호출됨!
		...
		// => 즉, 컴파일 단계에서의 실행 대상과 실행 단계에서의 실행 대상이 다를 수 있다!
		...
		car = dcar;
		car.drive(); // 오버라이딩된 메서드 호출 "디젤차를 운전하다!" 출력
		
		car = ecar;
		car.drive(); // 오버라이딩된 메서드 호출 "전기차를 운전하다!" 출력
	}
